...
class Sql:
 ...
 #### TABLO OUŞTURMA ^^^######   
    ...
    def SelectOpsWl_Series(esitmi):
        connection = sqlite3.connect(Paths.relative_to_data("siseriesdb.db"))
        cursor = connection.execute("select seriesName from watchlist_series where SeriesName = '{mi}'".format(mi = esitmi))
        a = []
        for row in cursor:
            a.append(row)
        if len(a) != 0:
